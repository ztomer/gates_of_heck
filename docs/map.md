...
| `local_ci.sh` | Declarative step runner: steps from `GOH_CI_STEPS` + `--step`. Fail accumulator, logs on failure. | opt-in | `test_local_ci.py` |
...
merge only. New per-language logic goes in the engine; new floor/CLI
semantics go in `coverage_gate.sh`.
...
| `mcp_scaffold.py` | stdio MCP JSON-RPC scaffold. | `test_mcp_scaffold.py` |
